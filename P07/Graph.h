...
public:
	...
	MutablePriorityQueue<Edge<T> > readGraphIntoHeapArray();
};
...
template <class T>
MutablePriorityQueue<Edge<T>> Graph<T>::readGraphIntoHeapArray()
{
...
vector<Vertex<T>*> Graph<T>::calculateKruskal() {
	...
	vector<Vertex<T>*> ret;
	/*
	MutablePriorityQueue<Edge<T>> h = readGraphIntoHeapArray();
	h.buildHeap();
	DisjSet<Vertex<T>> s = new DisjSet(NUM_VERTICES);
